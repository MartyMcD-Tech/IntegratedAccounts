...
// Integrated Accounts — Quote Wizard (Typeform-style, v4.1, robust boot)

(function(){
  /* -------------------- BOOTSTRAP -------------------- */
  function start() {
    try { console.info("[IA] wizard v4.1 boot"); } catch {}
  ...
  function run(root){
    ...
    const IA_PRICE = (() => {
      ...
      const EXTRAS = {
        ...
        cisMonthly: 25, confirmationStatementAnnual: 60,
        propertyPct: 15, mtdItsaPerTaxpayer: 10
      };
      const money = (n) => Math.round((n + Number.EPSILON) * 100) / 100;
      const pct   = (base, p) => base * (p / 100);
      const findInvoicePrice = b => (INVOICE_PRICE.find(r=>r.band===b)?.price)||0;
    ...
    function render(){
      ...
      if (node.tp==="select") {
        ...
        getSupabaseEnv().then(env => {
          if (!env.configured) { saveBtn.disabled = true; saveBtn.title = "Supabase not configured on this deployment"; }
        });
